...
#application_train.to_pickle('application_train.pkl')
#application_test.to_pickle('application_test.pkl')
...
inc_by_edu = df[['AMT_INCOME_TOTAL', 'NAME_EDUCATION_TYPE']].groupby('NAME_EDUCATION_TYPE').median()['AMT_INCOME_TOTAL']
inc_by_occ = df[['AMT_INCOME_TOTAL', 'OCCUPATION_TYPE']].groupby('OCCUPATION_TYPE').median()['AMT_INCOME_TOTAL']
...
df['NEW_GOODS_TO_INC'] = df['AMT_GOODS_PRICE']/(1+df['AMT_INCOME_TOTAL'])
df['NEW_INC_BY_EDU'] = df['NAME_EDUCATION_TYPE'].map(inc_by_edu)
df['NEW_MEMBER_TO_INC'] = df['CNT_FAM_MEMBERS']/(1+df['AMT_INCOME_TOTAL'])
...
bureau = bureau.groupby('SK_ID_CURR',as_index = False).apply(lambda x: x.sort_values('DAYS_CREDIT', ascending=True)).reset_index(drop = True)
dif_credit_sum = bureau.groupby('SK_ID_CURR').AMT_CREDIT_SUM.transform(lambda x: x.diff().shift(-1))

bureau['DIF_CREDSUM'] = dif_credit_sum
...
num_aggregations = {
    ...
    'AMT_ANNUITY': ['max', 'mean', 'var', 'sum'],
    'CNT_CREDIT_PROLONG': ['sum','mean'],
    'MONTHS_BALANCE_MIN': ['min','mean'],
    'MONTHS_BALANCE_MAX': ['max','mean'],
    'MONTHS_BALANCE_SIZE': ['mean', 'sum'],
    'DIF_CREDSUM':['min', 'max', 'mean','var']
}

# Bureau and bureau_balance categorical features
cat_aggregations = {}
for cat in bureau_cat: cat_aggregations[cat] = ['mean', 'var']
for cat in bb_cat: cat_aggregations[cat + "_MEAN"] = ['mean', 'var']
...
bureau_agg = pd.concat([bureau_agg, active_agg], axis=1, join_axes=[bureau_agg.index])
...
bureau_agg = pd.concat([bureau_agg, closed_agg], axis=1, join_axes=[bureau_agg.index])
for e in cols:
...
previous_application = previous_application.groupby('SK_ID_CURR',as_index = False).apply(lambda x: x.sort_values('DAYS_DECISION', ascending=True)).reset_index(drop = True)
dif_amount = previous_application.groupby('SK_ID_CURR').AMT_APPLICATION.transform(lambda x: x.diff().shift(-1))
dif_annual = previous_application.groupby('SK_ID_CURR').AMT_ANNUITY.transform(lambda x: x.diff().shift(-1))


previous_application['DIF_AMOUNT'] = dif_amount
previous_application['DIF_ANNUAL'] = dif_annual

prev, cat_cols = one_hot_encoder(previous_application, nan_as_category= False)
...
prev['ANNUITY_PERC'] = prev['AMT_ANNUITY']/prev['AMT_CREDIT']

# Previous applications numeric features
num_aggregations = {
    'AMT_ANNUITY': ['min', 'max', 'mean', 'var'],
    'AMT_APPLICATION': ['min', 'max', 'mean','var', 'sum'],
    ...
    'ANNUITY_PERC' : ['min' , 'max' , 'mean'],
    'DIF_AMOUNT' : ['min', 'max', 'mean','var'],
    'DIF_ANNUAL' : ['min', 'max', 'mean','var']
}
...
prev_agg = pd.concat([prev_agg, approved_agg], axis=1, join_axes=[prev_agg.index])
# Previous Applications: Refused Applications - only numerical features
refused = prev[prev['NAME_CONTRACT_STATUS_Refused'] == 1]
refused_agg = refused.groupby('SK_ID_CURR').agg(num_aggregations)
refused_agg.columns = pd.Index(['REFUSED_' + e[0] + "_" + e[1].upper() for e in refused_agg.columns.tolist()])
#prev_agg = prev_agg.join(refused_agg, how='left', on='SK_ID_CURR')
prev_agg = pd.concat([prev_agg, refused_agg], axis=1, join_axes=[prev_agg.index])
...
pos, cat_cols = one_hot_encoder(POS_CASH_balance, nan_as_category= False)
# Features
aggregations = {
    'MONTHS_BALANCE': ['max', 'mean', 'size'],
    'SK_DPD': ['max', 'mean', 'var'],
    'SK_DPD_DEF': ['max', 'mean', 'var'],
    'CNT_INSTALMENT' : ['min','max','mean','var'],
    'CNT_INSTALMENT_FUTURE' : ['min','max','mean','var']
}
for cat in cat_cols:
...
aggregations = {
    ...
    'DPD': ['min', 'max', 'mean', 'sum', 'var'],
    'DBD': ['min' , 'max', 'mean', 'sum', 'var'],
...
credit_card_balance['DRAWING_TOTAL'] = credit_card_balance['AMT_DRAWINGS_ATM_CURRENT'] + credit_card_balance['AMT_DRAWINGS_CURRENT'] + credit_card_balance['AMT_DRAWINGS_OTHER_CURRENT'] + \
credit_card_balance['AMT_DRAWINGS_POS_CURRENT']

credit_card_balance['DRAWING_NUM'] = credit_card_balance['CNT_DRAWINGS_ATM_CURRENT'] + credit_card_balance['CNT_DRAWINGS_CURRENT'] + credit_card_balance['CNT_DRAWINGS_OTHER_CURRENT'] + \
credit_card_balance['CNT_DRAWINGS_POS_CURRENT']


credit_card_balance = credit_card_balance.groupby('SK_ID_CURR',as_index = False).apply(lambda x: x.sort_values('MONTHS_BALANCE', ascending=True)).reset_index(drop = True)
dif_balance = credit_card_balance.groupby('SK_ID_CURR').DRAWING_TOTAL.transform(lambda x: x.diff().shift(-1))
dif_drawings = credit_card_balance.groupby('SK_ID_CURR').DRAWING_NUM.transform(lambda x: x.diff().shift(-1))
dif_payment= credit_card_balance.groupby('SK_ID_CURR').AMT_PAYMENT_CURRENT.transform(lambda x: x.diff().shift(-1))
dif_payment_total = credit_card_balance.groupby('SK_ID_CURR').AMT_PAYMENT_TOTAL_CURRENT.transform(lambda x: x.diff().shift(-1))
dif_recei_principle = credit_card_balance.groupby('SK_ID_CURR').AMT_RECEIVABLE_PRINCIPAL.transform(lambda x: x.diff().shift(-1))
dif_recei = credit_card_balance.groupby('SK_ID_CURR').AMT_RECIVABLE.transform(lambda x: x.diff().shift(-1))
dif_recei_total = credit_card_balance.groupby('SK_ID_CURR').AMT_TOTAL_RECEIVABLE.transform(lambda x: x.diff().shift(-1))

credit_card_balance['DIF_BALANCE'] = dif_balance
credit_card_balance['DIF_DRAW'] = dif_drawings
credit_card_balance['DIF_PAY'] = dif_payment
credit_card_balance['DIF_PAY_TOTAL'] = dif_payment_total
credit_card_balance['DIF_RECEI_PRINCIPLE'] = dif_recei_principle
credit_card_balance['DIF_RECEI'] = dif_recei
credit_card_balance['DIF_RECEI_TOTAL'] = dif_recei_total
...
def kfold_lightgbm(df, num_folds, stratified = False, debug= False):
    ...
    if not debug:
        ...
        sub_df[['SK_ID_CURR', 'TARGET']].to_csv('02.csv', index= False)
